...
type Notificacion = {
  ...
  origen: 'anuncio' | 'ingreso' | 'contacto' | 'cita';
  originalId?: number;
};
  ...
  const cargarNotificaciones = async () => {
    try {
      ...
      const endpoints = [
        ...
        { url: '/api/contactarnos', tipo: 'contacto', prefix: 'con' },
        { url: '/api/citas', tipo: 'cita', prefix: 'cit' }
      ];
      ...
      for (const { data, tipo, prefix } of responses) {
        ...
        switch (tipo) {
          ...
          case 'contacto':
            limitData.forEach((contacto: any) => {
              if (!contacto.idcontactarnos) return;
              todasNotificaciones.push({
                id: generateUniqueId(prefix, contacto.idcontactarnos),
                ...
                tipo: 'informativa',
                origen: 'contacto',
                originalId: contacto.idcontactarnos
              });
            });
            break;

          case 'cita':
            limitData.forEach((cita: any) => {
              if (!cita.idcita) return;
              todasNotificaciones.push({
                id: generateUniqueId(prefix, cita.idcita),
                titulo: `Cita ${cita.estado === 'pendiente' ? 'pendiente' : 'programada'}`,
                mensaje: `Para el ${cita.fecha} - ${cita.motivo}`,
                fecha: cita.fecha,
                leida: false,
                tipo: cita.estado === 'pendiente' ? 'urgente' : 'informativa',
                origen: 'cita',
                originalId: cita.idcita
              });
            });
            break;
        }
